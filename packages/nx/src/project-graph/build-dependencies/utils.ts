import { ESMap, isWhiteSpaceLike, MapLike, Push, ReadonlyESMap, SortedArray, SortedReadonlyArray, UnderscoreEscapedMap, __String } from 'typescript';
...
export function relativeComplement<T>(
  ...
  loopB: for (
    ...
    //   Debug.assertGreaterThanOrEqual(
        ...
        // Comparison.EqualTo
    //   );
        throw new Error('arrayB is not sorted');
    }
    ...
    loopA: for (const startA = offsetA; offsetA < arrayA.length; offsetA++) {
      if (offsetA > startA) {
        ...
        //   Comparison.EqualTo
        // );
        throw new Error('arrayA is not sorted');
      }
...
export function reduceLeft<T>(
  ...
  if (array && array.length > 0) {
    ...
    if (size > 0) {
      ...
      const end =
        count === undefined || pos + count > size - 1 ? size - 1 : pos + count;
      let result: T;
...
const createUIStringComparer = (() => {
  ...
  function createFallbackStringComparer(): Comparer<string> {
    ...
    function compareDictionaryOrder(a: string, b: string) {
      return (
        compareStrings(a.toUpperCase(), b.toUpperCase()) || compareStrings(a, b)
      );
    }
