...
describe('unittests:: services:: PreProcessFile:', () => {
  const preProcessFile = createPreProcessFile(ts);

  function test(
    sourceText: string,
    readImportFile: boolean,
    detectJavaScriptImports: boolean,
    expectedPreProcess: PreProcessedFileInfo
  ): void {
    const resultPreProcess = preProcessFile(
      sourceText,
      readImportFile,
    ...
    expect(resultPreProcess.isLibFile).toEqual(expectedPreProcess.isLibFile);
    ...
    expect(resultPreProcess.ambientExternalModules).toEqual(
